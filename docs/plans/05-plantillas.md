...
| 16 | **`revalidateTag` en todas las mutaciones** que persisten en DB, incluidas las de valores (renombrar criterio, cambiar tipo, reordenar, editar metadata o configuración de la plantilla). | Llamar `revalidateTag` solo en mutaciones estructurales | Decisión revisada durante verificación post-ejecución: sin `revalidateTag`, recargar la página devuelve el valor viejo desde `unstable_cache` aunque el cliente ya muestra "Guardado". El riesgo de Plan 4 (stale props que pisan el input) no aplica aquí porque los formularios sincronizan desde `updateState`, no desde props vía `useEffect([prop])`. |
...
- `updateCriterio` llama `revalidateTag` para que un reload posterior muestre el valor guardado. Esto es seguro porque la sincronización se hace desde `actionState`, no desde props, por lo que el soft-refresh no pisa el input del usuario.
