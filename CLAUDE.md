...
55. **Unión discriminada en props de componente: usar un campo literal explícito (`mode: "a" | "b"`), no inferir el modo por qué props opcionales vinieron seteadas**: al genericizar `CriteriosDescripcionesForm` para soportar dos modos de envío (guardado inmediato vía server action vs. bundleado en el submit de un formulario padre), la primera versión tipó la unión como `{ postaId: string; updateAction: Fn; onChange?: never } | { postaId?: never; updateAction?: never; onChange: Fn }`. TypeScript no narrowea de forma confiable con `"prop" in objeto` ni desestructurando cuando ambas ramas técnicamente declaran la misma key (una requerida, la otra `?: never`) — el compilador no descarta la rama `never` solo porque el valor esté ausente en runtime. Solución: agregar un campo discriminante literal (`mode: "persist" | "controlled"`) y branchear sobre `props.mode === "..."` sin desestructurar antes del chequeo; ahí sí el narrowing de TypeScript funciona de forma confiable. (Ver Plan 16, adenda post-ejecución).
