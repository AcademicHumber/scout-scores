...
54. **Ownership por rol (`actorRole` en repos) puede quedar sin una vía de UI que la ejercite**: Plan 16 agregó el chequeo `POSTA_NO_PROPIA` a `updatePosta`/`deletePosta` para que un `JUEZ` no edite la posta de otro, pero el plan no incluyó una página de edición para jueces — `/admin/postas/[id]` (el único lugar donde se llama a esas funciones) sigue siendo `requireRole(["ADMIN"])`. La regla de negocio está protegida y cubierta por tests de repositorio, pero un `JUEZ` no tiene hoy ningún flujo de UI real desde el que dispararla. Si se agrega una vista de "mis postas" para jueces en un plan futuro, ya no hace falta tocar el repositorio — el ownership ya está implementado ahí.
